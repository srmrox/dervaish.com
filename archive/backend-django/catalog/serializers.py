...
class RenditionRefSerializer(serializers.ModelSerializer):
    """Compact rendition reference for embedding in library/queue/search responses."""
    ...
    class Meta:
        model = Rendition
        fields = ("slug", "title", "kalam_slug", "kalam_title", "duration_ms", "has_media")

    def get_has_media(self, obj) -> bool:
        """True if some active mirror carries a variant (or the asset has a source
        URL). The client's device-side mirror toggles refine this at play time."""
        from federation.services import resolve_mirror_urls
        ...
        for asset in obj.media_assets.all():
            if asset.source_url:
                return True
            for v in asset.variants.all():
                if v.url or (v.storage_key and resolve_mirror_urls(v.storage_key, asset)):
                    return True
        return False
