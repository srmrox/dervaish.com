...
# Local dev origins are allowed by default so the Vite app works out of the box
# (whether it calls the API absolutely or via the proxy). Override in prod.
...
MEDIA_URL = "/media/"
# The media library lives at the repo root (moved out of backend/). Point it
# anywhere with DERVAISH_MEDIA_ROOT — e.g. an external drive or a shared folder.
MEDIA_ROOT = Path(os.getenv("DERVAISH_MEDIA_ROOT", str(BASE_DIR.parent / "mediafiles")))

# --- Local mirror mode -------------------------------------------------------
# A self-hostable, offline-replicable instance. When DERVAISH_LOCAL_MODE=true the
# local media mirror is treated as carrying the whole catalogue (carries_all), so
# an offline copy serves all media from disk. LOCAL_MEDIA_BASE_URL is the mirror's
# base (relative "/media/" works behind the Vite proxy or same-origin prod).
# SERVE_MEDIA_LOCALLY lets Django serve MEDIA_ROOT itself (dev / small self-hosts);
# larger deployments front it with nginx/whitenoise/CDN instead.
DERVAISH_LOCAL_MODE = env_bool("DERVAISH_LOCAL_MODE", False)
LOCAL_MEDIA_BASE_URL = os.getenv("LOCAL_MEDIA_BASE_URL", MEDIA_URL)
SERVE_MEDIA_LOCALLY = env_bool("SERVE_MEDIA_LOCALLY", DEBUG or DERVAISH_LOCAL_MODE)
STORAGES = {
